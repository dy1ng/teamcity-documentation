...
>* Large TeamCity installations might benefit from the fine-tuning of memory settings. The amount of memory allocated to the JVM of the TeamCity server should not exceed 60% of total available physical memory on the machine (to allow for nested process and OS-level caches usage). Also, on a machine with many CPU cores (for example, more than 8) where current CPU usage is below 60%, and heap (`–Xmx`) is set to more than 8 GB, enabling G1 JVM garbage collector through `-XX:+UseG1GC` might reduce the length of stop-the-world GC pauses. Note that starting with Java 9, the usage of G1GC is enabled by default, so you only need to add the `-XX:+UseG1GC` option if you use Java 8 (up to and including).
